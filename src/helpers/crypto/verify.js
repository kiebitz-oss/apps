...
export async function verify(keys, signedData){
	...
	for(let keyData of keys){
		...
		try {
			...
			const key = await e(crypto.subtle.importKey("spki", ab, {name: "ECDSA", namedCurve: "P-256"}, false, ["verify"]))
			const result = await e(crypto.subtle.verify({name: "ECDSA", hash: "SHA-256"}, key, signature, data))
			if (result === true){
				return true
			}
		} catch(e) {
