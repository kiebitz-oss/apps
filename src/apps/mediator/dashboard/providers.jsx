...
async function providers(state, keyStore, settings, keyPairs){
    ...
    try {
        const providersList = await e(backend.appointments.getPendingProviderData(keyPairs, 10))
...
const Providers = withRouter(withActions(({router, action, id, keyPairs, providers, providersAction}) => {
    ...
    if (providers === undefined || providers.status === "loading")
        return <F>
        <Message type="info"><T t={t} k="providers.loading" /></Message>
    </F>

    if (providers.status === "failed")
        return <F>
        <Message type="danger"><T t={t} k="providers.failed" /></Message>
    </F>

    const showProvider = (i) => router.navigateToUrl(`/mediator/providers/show/${i}`)

    let modal

    const closeModal = () => router.navigateToUrl("/mediator/providers")
    const confirmProvider = () => router.navigateToUrl("/mediator/providers")
    ...
    const providerItems = providers.list.map((provider, i) => <ListItem onClick={() => showProvider(i)}key={i} isCard>
        <ListColumn>
            {provider.data.info.name}
        </ListColumn>        
    </ListItem>)
