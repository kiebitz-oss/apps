...
import { setup, uploadSettings, uploadContactData, storeTracingData } from "./actions";
...
const PrintView = () => {
    ...
    generateTraceData.encryptedTraces.forEach((tr, i) => {
        ...
        QRCode.toCanvas(refs[i].current, `https://s.kiebitz.eu/#${base64Data}`, function (error) {
		  if (error)
		  	console.error(error)
        })
    })
...
*/
const Finalize = withSettings(withActions(
    ({
    	settings,
        setup,
        uploadContactData,
		...
		return <React.Fragment>
			<CardContent>
		        ...
		        <T t={t} k="finalize.text" link={<A key="letUsKnow" external href={settings.get("supportEmail")} ><T t={t} k="wizard.letUsKnow" key="letUsKnow"/></A>} />
		        </p>
		        <div className="kip-finalize-box">
		        	<ul>
                        <li>test {storeTracingData && storeTracingData.status}</li>
		        	</ul>
		        </div>
		    ...
		    <CardFooter>
		        <Button type="success"><T t={t} k="wizard.leave" /></Button>
		    </CardFooter>
		</React.Fragment>
	},
