...
import { withRouter, withSettings, withForm, withActions, Form as FormComponent, FieldSet, RetractingLabelInput, ErrorFor, T, CardFooter, CardContent, SubmitField, Button } from "components"
...
const BaseSettings = ({
    ...
    const controls = <React.Fragment>
        ...
        <h2><T t={t} k="provider-data.optional.title" /></h2>
        <p><T t={t} k="provider-data.optional.text" /></p>
        ...
        <div className="kip-cm-provider-data">
            <FormComponent onSubmit={onSubmit}>
                <FieldSet disabled={submitting}>
                    {
                        <React.Fragment>
                        	<h2><T t={t} k="saveAndRestore" /></h2>
                        	<h2><T t={t} k="providerData" /></h2>
                            {controls}
                            <SubmitField
                                disabled={!valid}
                                type={"success"}
                                ...
                                title={
                                    redirecting ? (
                                        <T t={t} k="provider-data.success" />
                                    ) : submitting ? (
                                        <T t={t} k="provider-data.saving" />
                                    ) : (
                                        <T t={t} k={"provider-data.save-and-continue"} />
                                    )
                                }
                            />
                        </React.Fragment>
...
const Settings = withActions(withForm(withRouter(withSettings(BaseSettings)), SettingsForm, "form"), [])
