...
import { keys, keyPairs, createAppointment, updateAppointment, cancelAppointment, openAppointments } from '../actions';
...
const AppointmentOverview = withActions(
    ({ openAppointmentsAction, cancelAppointmentAction, appointment, action, onClose, ...props }) => {
        ...
        return (
            <Modal bare onClose={onClose} {...props} className="kip-appointment-overview">
                <Card>
                    <CardHeader>
                    ...
                    <CardContent>
                        <ul className="kip-appointment-details">
                            ...
                            <li>
                                <T t={t} k="appointment-overview.details.slots" />: {appointment.slotData.length}{' '}
                            </li>
                            <li>
                        ...
                        {(acceptedItems.length > 0 && (
                            <F>
                                <h3>
                                    <T t={t} k="appointment-overview.details.booking-codes" />
                                </h3>
                    ...
                    <CardFooter>
                        <Button type="warning" href={`/provider/schedule/${action}/edit/${hexId}`}>
...
const AppointmentCard = withRouter(({ router, action, secondaryAction, id, appointment, n }) => {
    const p = Math.floor((appointment.duration / 60) * 100);
    const w = Math.floor(100 / (1 + appointment.maxOverlap));
    const y = Math.floor((new Date(appointment.timestamp).getMinutes() / 60) * 100);
    const i = appointment.overlapsWith.filter((oa) => oa.index < appointment.index).length;
    ...
    return (
        <div
            ...
            onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                if (!active) router.navigateToUrl(`/provider/schedule/${action}/show/${hexId}`);
            }}
        ...
        >
            ...
            {!tiny && (
                <F>
                    <span className="kip-tag kip-is-open kip-is-tiny">{appointment.slots}</span>
...
const CalendarAppointments = ({ action, secondaryAction, id, appointments }) => {
...
const HourRow = withRouter(({ appointments, action, router, secondaryAction, id, date, day, hour }) => {
    const ots = new Date(
        date.toLocaleDateString('en-US') + ' ' + hour.toLocaleString('en-US', { minimumIntegerDigits: 2 }) + ':00:00'
    );
    ...
    return (
        <div onClick={showNewAppointment} className={'kip-hour-row' + (hasAppointments ? ' kip-has-appointments' : '')}>
            {hasAppointments && (
...
const DayColumn = ({ day, date, fromHour, toHour, action, secondaryAction, id, appointments }) => {
    const hourRows = [<DayLabelRow appointments={appointments} date={date} day={day} key="-" />];
...
const WeekCalendar = withRouter(({ action, router, secondaryAction, id, startDate, appointments }) => {
    let fromHour;
    let toHour;
    ...
    appointments.forEach((app) => {
        ...
        const appEndDate = new Date(new Date(app.timestamp).getTime() + 1000 * 60 * app.duration);
        if (appStartDate < startDate || appStartDate > endDate || app.slots === 0) return;
        const startHours = appStartDate.getHours();
        const endHours = appEndDate.getHours();
        if (fromHour === undefined || startHours < fromHour) fromHour = startHours;
    ...
    const dayColumns = [<DayLabelColumn fromHour={fromHour} toHour={toHour} key="-" appointments={appointments} />];
    ...
    const goBackward = () => {
        const newDate = formatDate(getMonday(new Date(startDate.getTime() - 1000 * 60 * 60 * 24 * 7)));
...
const AppointmentItem = ({ appointment }) => {
    const acceptedItems = appointment.slotData
        .sort((a, b) => {
            if (a.open || b.open || a.token === undefined || b.token === undefined) return 0;
            try {
...
class AppointmentForm extends Form {
    validate() {
        const errors = {};
        if (this.data.date === undefined) errors.date = this.settings.t(t, 'new-appointment.please-enter-date');
        else if (this.data.time === undefined) errors.time = this.settings.t(t, 'new-appointment.please-enter-time');
        else {
            this.data.timestamp = new Date(`${this.data.date} ${this.data.time}`);
            if (this.data.timestamp < new Date()) errors.date = this.settings.t(t, 'new-appointment.in-the-past');
            // we allow appointments max. 30 days in the future
            if (this.data.timestamp > new Date(new Date().getTime() + 1000 * 60 * 60 * 24 * 30))
                errors.date = this.settings.t(t, 'new-appointment.too-far-in-the-future');
        }
        ...
        withRouter(
            withForm(
                ({
                    ...
                    const cancel = () => router.navigateToUrl(`/provider/schedule${actionUrl}`);

                    let appointment;

                    if (id !== undefined) appointment = appointments.find((app) => getHexId(app.id) === id);
                        ...
                        if (appointment !== undefined) {
                            ...
                            for (const [_, v] of Object.entries(properties)) {
                                for (const [kk, _] of Object.entries(v.values)) {
                                    if (appointment[kk] !== undefined) appointmentData[kk] = true;
                        ...
                        >
                            <FormComponent>
                                <FieldSet>
                                    ...
                                    <div className="kip-field kip-is-fullwidth">{apptProperties}</div>
                                </FieldSet>
                            </FormComponent>
                        </Modal>
                    );
                ...
                ({
                    ...
                    if (action !== undefined) {
                        ...
                        if (result) {
                            ...
                            startDate = getMonday(new Date(Number(year), Number(month) - 1, Number(day)));
                        }
                    }
                                ...
                                <CardContent>
                                    <div className="kip-non-printable">
                                        ...
                                        <DropdownMenu
                                            title={
                                                <F>
                                                    <Icon icon="calendar" /> <T t={t} k={`schedule.${view}`} />
                                                </F>
                                            }
                                        >
                                            <DropdownMenuItem icon="calendar" onClick={() => setView('calendar')}>
                                                <T t={t} k={`schedule.calendar`} />
                                            </DropdownMenuItem>
                                            <DropdownMenuItem icon="list" onClick={() => setView('booking-list')}>
                    ...
                    return <WithLoader resources={[keyPairs, openAppointments]} renderLoaded={render} />;
                },
